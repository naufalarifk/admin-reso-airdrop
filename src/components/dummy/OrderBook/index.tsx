...
export const OrderBook = ({data, ticker, currency, usdt}: {  data: {asks: string[][]; bids: [][]; timestamp: number}; ticker: MarketTicker; currency: Currencies; usdt: Currencies}) => {
      ...
      <div className="flex flex-col  overflow-hidden h-[442px]">
        ...
        >
          {asks?.map((order, i) => (
            ...
            >
              <div className="text-primary flex-1">{Decimal.format(order?.[0], precision, ',')}</div>
              <div className="text-soft flex-1 text-right">
              {Decimal.format(order?.[1], precision, ',')}
              </div>
              <div className="text-soft flex-1 text-right">{generateTotal(+order?.[0], +order?.[1])}</div>
        ...
        >
          ...
          <div className="text-xs font-normal text-soft">≈{tick?.last / +usdtPrice} USDT</div>
        </div>
        {/* End Ticker */}
        ...
        >
          {bids?.map((order, i) => (
            <div
              ...
              className="flex pr-1 h-[18px] justify-between text-[11.63px] relative"
            >
              <div className="text-success flex-1">{Decimal.format(order?.[0], precision, ',')}</div>
              <div className="text-soft flex-1 text-right">
              {Decimal.format(order?.[1], precision, ',')}
              </div>
              <div className="text-soft flex-1 text-right">{generateTotal(+order?.[0], +order?.[1])}</div>
