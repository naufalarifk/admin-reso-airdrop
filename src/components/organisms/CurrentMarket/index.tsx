...
export const CurrentMarket = ({
   ...
   // const getMarketIcon = parsePair(marketId!);
   //   console.log("BASE =>", getMarketIcon.base);
   //   console.log("MARKET BASE =>", market[0].base_unit);
   ...
   return (
      <section className="hidden lg:block">
         <div className="flex space-x-3">
            <IcCoinPairs className="h-10 w-10 rounded-lg bg-[#20232e] p-1" />
            {market?.map(e => (
               <div
                  key={e.base_unit}
                  onClick={() => navigate(`/swap/${e.name.replace('/', '-')}`)}
                  className={`${
                     marketId === e.name ? 'bg-[#20232e]' : 'border border-[#20232e] bg-transparent'
         ...
         <div className="my-2 flex items-center space-x-2">
            ...
            <div className="flex space-x-1 rounded-xl bg-[#FE9F00] bg-opacity-10 p-2">
               <svg
               ...
               <div>
                  ...
                  <Text className="font-semibold text-[#FE9F00]">This pair had low liquidity</Text>
               </div>
            </div>
            <div className="flex space-x-2">
               {timestamps.map(timestamp => (
                  <div className="rounded-lg bg-[#F23F5D] px-3 py-1">
                     <Text>{timestamp}</Text>
                  </div>
               ))}
            </div>
